...
class X11Window(Window):
    ...
    def do_initialize_x_backend(self):
        ...
        def on_draw(*_):
            ...
            self._xwindow.set_wm_class('draw', 'XlibExample')

            self.set_wm_normal_hints()
    ...
    def set_as_dock(self):
        if not self._xwindow or not self._xdisplay:
            logger.warning("X11 window is not initialized; cannot set as dock.")
            return

        atom_net_wm_window_type = self._xdisplay.intern_atom('_NET_WM_WINDOW_TYPE')
        atom_net_wm_window_type_dock = self._xdisplay.intern_atom('_NET_WM_WINDOW_TYPE_DOCK')
        ...
        atom_net_wm_strut_partial = self._xdisplay.intern_atom('_NET_WM_STRUT_PARTIAL')
        atom_net_wm_strut = self._xdisplay.intern_atom('_NET_WM_STRUT')

        screen_width = self._rectangle.width
        dock_height = 42

        strut = [0, 0, dock_height, 0,  # top, bottom, left, right
                 0, 0, 0, 0,           # start/end positions
                 0, 0, 0, 0]
        strut[2] = dock_height  # Espaço reservado na parte superior
        self._xwindow.change_property(atom_net_wm_strut_partial, Xatom.CARDINAL, 32, strut)
        self._xwindow.change_property(atom_net_wm_strut, Xatom.CARDINAL, 32, strut[:4])
    ...
    def set_wm_normal_hints(self):
        if not self._xwindow:
            logger.warning("X11 window is not initialized; skipping setting normal hints.")
            return

        hints = self._xwindow.get_wm_normal_hints()

        hints.flags = (
            Xutil.PPosition |  # Localização inicial
            Xutil.PSize |      # Tamanho inicial
            Xutil.PMinSize |   # Tamanho mínimo
            Xutil.PMaxSize |   # Tamanho máximo
            Xutil.PBaseSize    # Tamanho base
        )
        ...
        hints.min_width = 1920
        hints.min_height = 42

        hints.max_width = 1920
        hints.max_height = 42

        hints.base_width = 1920
        hints.base_height = 42

        self._xwindow.set_wm_normal_hints(hints)
        self._xdisplay.flush()
